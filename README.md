...
# Agent drafts product brief, requirements doc, and technical spec
...
# You run audits → identify issues → create work orders → audit the plan → implement → audit again
...
|-----------|-------|-------------|
| Check Scripts | 24 | Secrets, security, tests, code quality, architecture, roadmap alignment |
...
| Decision Trees | 8 | Product shape, tech stack, which checks to run, architecture rules |
...
5. Agent selects stack, checks, phases, and hooks
...
8. Agent generates DEVELOPMENT-PLAN.md (phased roadmap with ordered work items) — defines what's next
...
- "Under the hood, I ran the plan-acceptance check. That's the check that runs before we start building — it makes sure the plan is solid."
...
scripts/                         ← 24 check scripts (agent copies to your project)
...
- **Work order** — Checks work items have required sections
...
- **Personal data** — How you handle user data, privacy-style checks
...
- **Evidence bundle** — Proof for audits and compliance
...
### Runner (1)
- **Check runner** — Runs all the checks in the right order
...
- [PRODUCT-DISCOVERY.md](PRODUCT-DISCOVERY.md) — Discovery playbook for idea → requirements doc → spec
- [PROJECT-INTAKE.md](PROJECT-INTAKE.md) — Adaptive refinement questionnaire
- [docs/project-definition.schema.json](docs/project-definition.schema.json) — Project definition (what the agent knows about your project)
